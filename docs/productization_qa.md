...
- Landing/social metadata audit: project, demo, and guidebook pages keep Open Graph/Twitter card metadata, favicon links, and a nonblank 1200x630 social card so LinkedIn/Facebook shares render correctly.
- API cost gate audit: the public taste demo opens the web build with deterministic offline responses, the client has no local passcode comparison, voice access is server-token based, and ElevenLabs TTS is reached only after token validation.
